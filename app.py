...
def show_venue(venue_id):
  ...
  if not venue:
    ...
    data = {
      ...
      "past_shows": last_show,
      "past_shows_count": last_show_count,
...
def create_venue_submission():
  ...
  if not form.validate():
    ...
    try:
      ...
      for genre in genres:
        get_gen = Genre.query.filter_by(name=genre).one_or_none()
        if get_gen:
          new_venue.genres.append(get_gen)
        else:
          new_gen = Genre(name=genre)
          db.session.add(new_gen)
          new_venue.genres.append(new_gen)
...
def edit_venue_submission(venue_id):
  ...
  if not form.validate():
    ...
    try:
      ...
      venue.genres = []
      for genre in genres:
        get_genre = Genre.query.filter_by(
...
def create_artist_submission():
  ...
  if not form.validate():
    ...
    if not error:
      ...
      flash('ERROR: ' + name + ' could not be listed.')
      abort(500)
...
def create_show_submission():
  ...
  if error:
    flash(f'ERROR:   Show not have listed!')
  else:
